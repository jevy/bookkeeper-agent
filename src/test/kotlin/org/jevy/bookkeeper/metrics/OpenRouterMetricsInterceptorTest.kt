...
class OpenRouterMetricsInterceptorTest {
    ...
    fun `records cost and token metrics from OpenRouter response`() {
        ...
        val cost = registry.counter("bookkeeper.openrouter.cost", "provider", "Google").count()
        assertEquals(0.000177, cost, 0.000001)

        // Zero-value token counters should not be incremented
        assertEquals(0.0, registry.counter("bookkeeper.openrouter.tokens.cached").count())
        assertEquals(0.0, registry.counter("bookkeeper.openrouter.tokens.cache_write").count())
        assertEquals(0.0, registry.counter("bookkeeper.openrouter.tokens.reasoning").count())
    }
    ...
    fun `records non-zero token metrics`() {
        ...
        assertEquals(0.005, registry.counter("bookkeeper.openrouter.cost", "provider", "Anthropic").count(), 0.000001)
        assertEquals(1500.0, registry.counter("bookkeeper.openrouter.tokens.cached").count())
        assertEquals(3000.0, registry.counter("bookkeeper.openrouter.tokens.cache_write").count())
        assertEquals(200.0, registry.counter("bookkeeper.openrouter.tokens.reasoning").count())
    }
    ...
    fun `gracefully handles missing usage block`() {
        ...
        assertEquals(0.0, registry.counter("bookkeeper.openrouter.cost", "provider", "unknown").count())
    ...
    fun `uses unknown provider when provider field is missing`() {
        ...
        assertEquals(0.001, registry.counter("bookkeeper.openrouter.cost", "provider", "unknown").count(), 0.000001)
    }
}
