    ...
    private fun recordMetrics(bytes: ByteArray) {
        try {
            ...
            if (cost != null) {
                ...
                meterRegistry.counter("tiller.openrouter.cost", "provider", provider).increment(cost)
            }
            ...
            if (promptDetails != null) {
                ...
                if (cached != null && cached > 0) {
                    meterRegistry.counter("tiller.openrouter.tokens.cached").increment(cached)
                }
                val cacheWrite = promptDetails.get("cache_write_tokens")?.asDouble
                if (cacheWrite != null && cacheWrite > 0) {
                    meterRegistry.counter("tiller.openrouter.tokens.cache_write").increment(cacheWrite)
                }
            }
            ...
            if (completionDetails != null) {
                ...
                if (reasoning != null && reasoning > 0) {
                    meterRegistry.counter("tiller.openrouter.tokens.reasoning").increment(reasoning)
                }
            }
        } catch (e: Exception) {
